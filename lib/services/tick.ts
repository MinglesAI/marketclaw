...
export async function projectTick(opts: {
  ...
  for (const role of roles) {
    ...
    // Publish policy gate: control publisher dispatch based on publishPolicy
    if (role === "publisher") {
      const policy = workflow.publishPolicy ?? PublishPolicy.AGENT;
      if (policy === PublishPolicy.HUMAN) {
        skipped.push({ role, reason: "Publish policy: human (waiting for human to manually trigger)" });
        continue;
      }
      if (policy === PublishPolicy.SKIP) {
        skipped.push({ role, reason: "Publish policy: skip (heartbeat handles via publish-skip pass)" });
        continue;
      }
      // PublishPolicy.AGENT: dispatch publisher automatically (fall through)
    }
    ...
    if (role === "publisher") {
      ...
      if (routing === "skip") {
        skipped.push({ role, reason: "publish:skip label" });
        continue;
      }
    }
