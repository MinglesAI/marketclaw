...
import {
  ...
  performTestSkipPass,
  performPublishSkipPass,
  performAnalyticsTriggerPass,
} from "./passes.js";
...
export type TickResult = {
  ...
  totalPublishSkipTransitions: number;
  totalAnalyticsTriggers: number;
};
...
export async function tick(opts: {
  ...
  if (slugs.length === 0) {
    return {
      ...
      totalPublishSkipTransitions: 0,
      totalAnalyticsTriggers: 0,
    };
  }
  ...
  for (const slug of slugs) {
    try {
      ...
      // Publish skip pass: auto-transition publish:skip issues through the publish queue
      result.totalPublishSkipTransitions += await performPublishSkipPass(
        workspaceDir, slug, provider, resolvedConfig,
      );

      // Analytics trigger pass: auto-transition published issues to analytics queue after N days
      result.totalAnalyticsTriggers += await performAnalyticsTriggerPass(
        workspaceDir, slug, provider, resolvedConfig,
      );
  ...
  await auditLog(workspaceDir, "heartbeat_tick", {
    ...
    publishSkipTransitions: result.totalPublishSkipTransitions,
    analyticsTriggers: result.totalAnalyticsTriggers,
