...
async function processAllAgents(
  ...
  for (const { agentId, workspace } of agents) {
    ...
    result.totalPublishSkipTransitions += agentResult.totalPublishSkipTransitions;
    result.totalAnalyticsTriggers += agentResult.totalAnalyticsTriggers;
  }
...
function logTickResult(
  ...
  if (
    ...
    result.totalPublishSkipTransitions > 0 ||
    result.totalAnalyticsTriggers > 0
  ) {
    logger.info(
      ...
      `${result.totalTestSkipTransitions} test skips, ${result.totalPublishSkipTransitions} publish skips, ` +
      `${result.totalAnalyticsTriggers} analytics triggers, ${result.totalSkipped} skipped`,
    );
  }
}
