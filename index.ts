...
import { createTaskScheduleTool } from "./lib/tools/tasks/task-schedule.js";
...
const plugin = {
  ...
  name: "MarketClaw",
  description:
    "AI marketing team orchestration — Strategist, Creator, Reviewer, Publisher, Analyst pipeline with GitHub integration and content calendar.",
  configSchema: {
    type: "object",
    properties: {
      projectExecution: {
      ...
      work_heartbeat: {
        ...
        properties: {
          ...
          maxPickupsPerTick: {
            type: "number",
            default: 4,
            description: "Max worker dispatches per agent per tick.",
          },
        },
      },
    },
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.registerTool(createTaskScheduleTool(ctx), { names: ["task_schedule"] });
    ...
    api.logger.info(
      "MarketClaw plugin registered (24 tools, 1 CLI command group, 1 service, 2 hooks)",
    );
  },
};
