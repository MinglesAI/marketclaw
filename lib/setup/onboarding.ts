...
export async function hasWorkspaceFiles(
  ...
  try {
    ...
    return (content.includes("MarketClaw") || content.includes("DevClaw")) && (content.includes("task_start") || content.includes("work_start"));
  } catch {
    return false;
  }
}
...
export function buildReconfigContext(): string {
  ...
  return `# MarketClaw Reconfiguration

The user wants to reconfigure MarketClaw. Default model configuration:

${modelTable}

Models are configured in \`marketclaw/workflow.yaml\`. Edit that file directly or call \`setup\` with a \`models\` object to update.
...
4. **Publish policy** — edit \`marketclaw/workflow.yaml\` → \`publishPolicy: agent|skip|human\`
5. **Analytics delay** — edit \`marketclaw/workflow.yaml\` → \`analyzeAfterDays: 7\`
6. **Brand context** — fill in \`.agents/product-marketing-context.md\` in your campaign repo
...
export function buildOnboardToolContext(): string {
  ...
  return `# MarketClaw Onboarding

## What is MarketClaw?
MarketClaw turns each Telegram group into an autonomous AI marketing team:
- An **orchestrator** that manages campaigns and delegates content work
- **Strategist workers** (CMO-level research + campaign brief writing)
- **Creator workers** (junior/medior/senior content writers for posts, emails, articles)
- **Reviewer workers** (brand + quality review before publication)
- **Publisher workers** (publish to LinkedIn, Twitter, email, Telegram)
- **Analyst workers** (collect metrics 7 days post-publish, feed learnings into next campaign)
...
**Step 4: Brand Context Setup (IMPORTANT)**
After setup completes, explain the brand context file:

📝 **Brand Context File:**
MarketClaw agents read \`.agents/product-marketing-context.md\` from the campaign repo.
This is the MOST IMPORTANT config file — it contains brand voice, ICP, positioning, and messaging.

Ask: "Do you want to set up the brand context file now? It's the single most important file for content quality."
- If yes → guide them to fill in \`.agents/product-marketing-context.md\` in their repo
- If no → remind them to do it before running their first campaign

**Step 5: Telegram Group Setup (IMPORTANT)**
After setup completes, explain project isolation best practices:

📱 **Telegram Group Guidance:**
MarketClaw uses **one Telegram group per project** for isolation and clean backlogs.
...
- **Review policy**: human (default) — Content PRs need human approval on GitHub, heartbeat auto-merges when approved.
- **Publish policy**: agent (default) — Publisher worker automatically dispatched when content enters "To Publish" queue.
- **Analytics**: After \`analyzeAfterDays\` (default: 7) days post-publication, Analyst worker is dispatched automatically.
- **Customization**: They can change reviewPolicy (human/agent/skip), publishPolicy (agent/skip/human), or analyzeAfterDays in workflow.yaml.
- Say: "Your pipeline is: Strategy → Create → Review → Publish → [7 days] → Analyze → Done. All steps are automated — you just approve issues and content PRs."
