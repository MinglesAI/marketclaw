...
import { publishSkipPass } from "./publish-skip.js";
import { analyticsTriggerPass } from "./analytics-trigger.js";
...
/**
 * Run publish skip pass for a project — auto-transition publish:skip issues through the publish queue.
 */
export async function performPublishSkipPass(
  workspaceDir: string,
  projectSlug: string,
  provider: import("../../providers/provider.js").IssueProvider,
  resolvedConfig: ResolvedConfig,
): Promise<number> {
  return publishSkipPass({
    workspaceDir,
    projectName: projectSlug,
    workflow: resolvedConfig.workflow,
    provider,
...
/**
 * Run analytics trigger pass — auto-transition published issues to analytics queue after N days.
 */
export async function performAnalyticsTriggerPass(
  workspaceDir: string,
  projectSlug: string,
  provider: import("../../providers/provider.js").IssueProvider,
  resolvedConfig: ResolvedConfig,
): Promise<number> {
  return analyticsTriggerPass({
    workspaceDir,
    projectName: projectSlug,
    workflow: resolvedConfig.workflow,
    provider,
